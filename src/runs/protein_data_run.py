...
class ProteinDataRun:
    ...
    def __init__(
        ...
        self._selected_features: Optional[List[str]] = selected_features if selected_features else None
        self.n_features_to_select: Optional[int] = n_features_to_select if n_features_to_select else 0
        ...
        self.run_config.update(
            {
                ...
                "n_features_requested": getattr(
                    self.fs, "n_features_to_select", self.n_features_to_select
                ) if self.fs else self.n_features_to_select,
            }
        )
    ...
    def _df(self) -> pd.DataFrame:
        ...
        if self.target not in self.data.treated_data.columns:
            self.logger.error(f"Target column '{self.target}' not found in treated data columns: {self.data.treated_data.columns.tolist()}")
            raise ValueError(f"Target column '{self.target}' not found in treated data.")
    ...
    def selected_features_df(self) -> pd.DataFrame:
        ...
        if self._selected_features: # Features were provided in __init__
            self.logger.info(f"Using pre-defined selected features: {self._selected_features}")
            # Ensure all pre-selected features are in _X
            missing_features = [f for f in self._selected_features if f not in self._X.columns]
            if missing_features:
                self.logger.error(f"Pre-selected features not found in data: {missing_features}")
                raise ValueError(f"Pre-selected features not found in data: {missing_features}")
        ...
        self.logger.info("No feature selector provided and no pre-selected features. Using all available features.")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if selected_X_df.empty:
                self.logger.error("No features available for model training after selection.")
            ...
            if selected_X_df.shape[0] != target_y.shape[0]:
                self.logger.error(f"Shape mismatch between features ({selected_X_df.shape}) and target ({target_y.shape}).")
            ...
            self.run_config["selected_features"] = selected_X_df.columns.tolist() # Update run_config with actual features used


            self.logger.info(f"Selected features DataFrame head: {selected_X_df.head()}")
            ...
            self.logger.info(f"Fitting Model (type: {self.model.model_type}) with {selected_X_df.shape[1]} features.")
            ...
            stratify_option = target_y if len(target_y.unique()) > 1 and target_y.nunique() < len(target_y) // 2 else None
            if stratify_option is not None and stratify_option.value_counts().min() < 2 : # Check for scikit-learn min samples per class for stratify
                self.logger.warning(f"Target stratification might not be possible due to small class sizes. Disabling stratification. Value counts: {target_y.value_counts().to_dict()}")
                stratify_option = None

            X_train, X_test, y_train, y_test = train_test_split(
                scaled_X_df, target_y, test_size=0.2, random_state=42, stratify=stratify_option
            )
            self.logger.info(f"Data split into train/test. X_train shape: {X_train.shape}, X_test shape: {X_test.shape}")
            ...
            self.logger.info(f"Test Accuracy: {test_accuracy}, Test F1 Score: {test_f1}")
            ...
            result_data["scores"] = scores.tolist() if isinstance(scores, np.ndarray) else scores
            result_data["mean_cv_score"] = np.mean(scores) if scores is not None and (isinstance(scores, list) and len(scores) > 0 or isinstance(scores, np.ndarray) and scores.size > 0) else None
            ...
            self.logger.info(f"Run Success. Mean CV Score: {result_data['mean_cv_score']}")
